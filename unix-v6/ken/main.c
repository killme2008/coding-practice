 ...
 */
sureg() //将user的APR反应到硬件的APR
{
	register *up, *rp, a;

	a = u.u_procp->p_addr; //数据段的物理地址
	up = &u.u_uisa[16];
	rp = &UISA->r[16];
	if(cputype == 40) {
		up =- 8;
	...
	while(rp > &UISA->r[0])
		*--rp = *--up + a; //补正数据段的 APR 数据 PAR
	if((up=u.u_procp->p_textp) != NULL)
		a =- up->x_caddr;  //使用代码段的地址作为补正值
	up = &u.u_uisd[16];
	rp = &UISD->r[16];
	if(cputype == 40) {
		up =- 8;
...
estabur(nt, nd, ns, sep)
{
	...
	if(sep) {
		...
		if(nseg(nt)+nseg(nd)+nseg(ns) > 8)  //超过总页数
			goto err;
	if(nt+nd+ns+USIZE > maxmem)　　//超过内存上限
		goto err;
	a = 0;
	ap = &u.u_uisa[0];　//PAR
  dp = &u.u_uisd[0]; //PDR
	while(nt >= 128) {  //分配代码段，单位为64字节的长度nt
		*dp++ = (127<<8) | RO;
		*ap++ = a;
		a =+ 128;
		nt =- 128;
